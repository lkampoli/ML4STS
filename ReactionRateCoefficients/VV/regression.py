...
dataset_T=np.loadtxt("./data/N2/down/Temperatures.csv")
dataset_k=np.loadtxt("./data/N2/down/VV_DOWN_N2_RATES-0->1.csv")

dataset = "./data/N2/down/VV_DOWN_N2_RATES-0->1.csv"
print(dataset[32], dataset[35])

x = dataset_T.reshape(-1,1) # T [K]
y = dataset_k[:,0:1]        # Rci (relaxation source terms)
...
dump(sc_x, open('scaler_x_VV_DOWN_N2_RATES-0->1.pkl', 'wb'))
dump(sc_y, open('scaler_y_VV_DOWN_N2_RATES-0->1.pkl', 'wb'))

#dump(sc_x, open('scaler_x_VV_UP_N2_RATES-0->1.pkl', 'wb'))
#dump(sc_y, open('scaler_y_VV_UP_N2_RATES-0->1.pkl', 'wb'))
...
plt.savefig("regression_kNN_VV_DOWN_N2_RATES-0->1.pdf", dpi=150, crop='false')
plt.show()

# save the model to disk
dump(gs, 'model_kNN_VV_DOWN_N2_RATES-0->1.sav')
