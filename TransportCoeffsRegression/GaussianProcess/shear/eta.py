...
hyper_params = [{'alpha': (1e-3, 1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3,), 'n_restarts_optimizer': (0,1,10,100,),
...
print("GP complexity and bandwidth selected and model fitted in %.3f s" % runtime)
...
best_n_restarts_optimizer = grid_clf.best_params_['n_restarts_optimizer']
...
print("GP complexity and bandwidth selected and model fitted in %.3f s" % gp_fit, file=outF)
print("GP prediction for %d inputs in %.3f s" % (x_test.shape[0], gp_predict),file=outF)
