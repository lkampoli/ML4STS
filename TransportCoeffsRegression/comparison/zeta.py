...
# KRR
hyper_params = [{'kernel': ('poly','rbf',), 'alpha': (1e-4,1e-2,0.1,1,), 'gamma': (0.01,0.1,1,10,),}]
#hyper_params = [{'kernel': ('poly','rbf','sigmoid',), 'alpha': (1e-4,1e-2,0.1,1,), 'gamma': (0.01,0.1,1,10,),}]
#hyper_params = [{'kernel': ('linear', 'poly','rbf','sigmoid',), 'alpha': (1e-4,1e-2,0.1,1,), 'gamma': (0.01,0.1,1,10,),}]
#hyper_params = [{'kernel': ('rbf',), 'alpha': (1e-4,1e-2,0.1,1,), 'gamma': (0.01,0.1,1,10,),}]
#hyper_params = [{'kernel': ('rbf',), 'alpha': (0.1,), 'gamma': (0.1,),}]
...
#train_score_evs = explained_variance_score(sc_y.inverse_transform(y_train), sc_y.inverse_transform(grid_clf.predict(x_train)), multioutput='raw_values')
#test_score_evs  = explained_variance_score(sc_y.inverse_transform(y_test),  sc_y.inverse_transform(grid_clf.predict(x_test)),  multioutput='raw_values')

#train_score_me = max_error(sc_y.inverse_transform(y_train), sc_y.inverse_transform(grid_clf.predict(x_train)), multioutput='raw_values')
#test_score_me  = max_error(sc_y.inverse_transform(y_test),  sc_y.inverse_transform(grid_clf.predict(x_test)),  multioutput='raw_values')

#train_score_msle = mean_squared_log_error(sc_y.inverse_transform(y_train), sc_y.inverse_transform(grid_clf.predict(x_train)), multioutput='raw_values')
#test_score_msle  = mean_squared_log_error(sc_y.inverse_transform(y_test),  sc_y.inverse_transform(grid_clf.predict(x_test)),  multioutput='raw_values')
...
# kernel-ridge 1 [('alpha', 0.0001), ('gamma', 10), ('kernel', 'rbf')]
# 1.4552684121591815e-14 2.9512220539578408e-08	2.1036165374108866e-14	3.421143644353957e-08	232.9235200881958

kr = KernelRidge(kernel='rbf', gamma=10, alpha=0.0001)
...
plt.title('Bulk viscosity regression with KR')
plt.ylabel(r'$\zeta$ [Pa·s]')
plt.xlabel('T [K] ')
plt.legend()
plt.tight_layout()
#plt.savefig("zeta_SVR_KR_RF_kN_GP.pdf", dpi=150, crop='false')
plt.savefig("zeta_KR.pdf", dpi=150, crop='false')
plt.show()
