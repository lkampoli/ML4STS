...
print("ET complexity and bandwidth selected and model fitted in %.6f s" % runtime)
...
print("ET complexity and bandwidth selected and model fitted in %.6f s" % regr_fit)

t0 = time.time()
y_regr = regr.predict(x_test)
regr_predict = time.time() - t0
print("ET prediction for %d inputs in %.6f s" % (x_test.shape[0], regr_predict))

# open a file to append
outF = open("output.txt", "a")
print("ET complexity and bandwidth selected and model fitted in %.6f s" % regr_fit, file=outF)
print("ET prediction for %d inputs in %.6f s" % (x_test.shape[0], regr_predict),file=outF)
...
plt.scatter(x_test_dim[:,1], y_regr_dim[:], s=2, c='k', marker='*', label='Extra Trees')
plt.title('Shear viscosity regression with ET')
plt.ylabel(r'$\eta$ [Pa·s]')
plt.xlabel('T [K] ')
plt.legend()
plt.tight_layout()
plt.savefig("eta_ET.pdf", dpi=150, crop='false')
plt.show()
