...
dump(sc_x, open('scaler_x_NO.pkl', 'wb'))
dump(sc_y, open('scaler_y_NO.pkl', 'wb'))
...
#features = np.array(['T', 'P', '$X_{NO}$', '$X_{O2}$', '$X_{NO}$', '$X_N$', '$X_O$'])
...
with open('output_NO.log', 'w') as f:
...
plt.legend()
plt.tight_layout()
plt.savefig("regression_DT_NO.pdf", dpi=150, crop='false')
plt.show()

# save the model to disk
dump(gs, 'model_NO.sav')
