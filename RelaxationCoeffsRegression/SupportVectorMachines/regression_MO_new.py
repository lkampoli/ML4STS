...
pipe = Pipeline([('scaler', StandardScaler()), ('regr', MultiOutputRegressor(SVR()))])

hyper_params = [{
                 'reg__estimator__kernel': ('poly', 'rbf',),
                 'reg__estimator__gamma': ('scale', 'auto',),
                 'reg__estimator__C': (1.0, 10.,100.,),
                 'reg__estimator__epsilon': (0.01, 0.015,),
                 'reg__estimator__coef0': (0.0,),
                 }]
...
#t0 = time.time()
#gs_svr = gs_svr.predict(x_test)
#regr_predict = time.time() - t0
#print("Prediction for %d inputs in %.6f s" % (x_test.shape[0], regr_predict))

#mean_scores = np.array(gs_svr.cv_results_['mean_test_score'])
...
#                                                           coef0=0.0, degree=3, epsilon=0.1, gamma='auto', kernel='rbf',
#                                                           max_iter=-1, shrinking=True, tol=0.001, verbose=True), n_jobs=1))
#                ])
