...
class PINN:
    ...
    def __init__(self, x, n, rho, u, p, E, R, layers):
        ...
        e1_norm  = rho_norm*u_norm
        e2_norm  = rho_norm*u_norm*u_norm + p_norm
        e3_norm  = h_norm + u_norm*u_norm*0.5
        ...
        #            tf.reduce_sum(tf.square(self.rho_tf - self.rho_pred)) + \
        #            tf.reduce_sum(tf.square(self.p_tf   - self.p_pred))   + \
        #            tf.reduce_sum(tf.square(self.E_tf   - self.E_pred))   + \
        #            w*tf.reduce_sum(tf.square(self.e1)) + \
        #            w*tf.reduce_sum(tf.square(self.e2)) + \
        #            w*tf.reduce_sum(tf.square(self.e3))
        i = 0
        while i<48:
          ...
          #self.loss = self.loss + tf.reduce_sum(tf.square(self.n_tf[i] - self.n_pred[i])) + \
          #                        tf.reduce_sum(tf.square(self.R_tf[i] - self.R_pred[i])) + \
          #                        w*tf.reduce_sum(tf.square(self.en[i]))
          i = i + 1
