...
if strcmp(sw_arr,"Park")
  ...
  const ARR_D_data = read("arr_d_park.dat");
  const ARR_Z_data = read("arr_z_park.dat");
...
elseif strcmp(sw_arr,"Scanlon")
  ...
  const ARR_D_data = load("arr_d_scanlon.dat");
  const ARR_Z_data = load("arr_z_scanlon.dat");
...
const en2_i = en_vibr(1);
const en2_0 = en_vibr_0(1);
const eo2_i = en_vibr(2);
const eo2_0 = en_vibr_0(2);
const eno_i = en_vibr(3);
const eno_0 = en_vibr_0(3);
...
const FHO_data = cat(3,fho_data_n2,fho_data_o2,fho_data_no);
...
const vss_data = cat(3,vss_data_n2,vss_data_o2,vss_data_no);
...
const r0 = 0.5*[R0(1)+R0; R0(2)+R0; R0(3)+R0];
...
# N2-N2
const sigma0 = pi*R0(1)^2;                # i^2
const lambda0 = 1/(sqrt(2)*n0*sigma0);    # i
const Delta = lambda0;                    # i
