...
class PINN:
    ...
    def __init__(self, x, n, rho, u, p, E, R, layers):
        ...
        self.n_tf = [tf.placeholder(tf.float32, shape=[None, self.n[i].shape[1]]) for i in range(int(length))]
        self.R_tf = [tf.placeholder(tf.float32, shape=[None, self.R[i].shape[1]]) for i in range(int(length))]
        ...
        n_norm  = [np.amax(n[i])    for i in range(int(length))]
        en_norm = [n_norm[i]*u_norm for i in range(int(length))]
        R_norm  = [np.amax(R[i])    for i in range(int(length))]
    ...
    def net_Euler_STS(self, x):
        ...
        n = [nci_rho_u_p_E[:,i:i+1] for i in range(int(length))]

        rho = nci_rho_u_p_E[:,length:length+1]
        u   = nci_rho_u_p_E[:,length+1:length+2]
        p   = nci_rho_u_p_E[:,length+2:length+3]
        E   = nci_rho_u_p_E[:,length+3:length+4]

        R = [nci_rho_u_p_E[:,i+1+4:i+1+5] for i in range(int(length))]

        n_u_x = [tf.gradients(n[i] *u, x)[0] for i in range(int(length))]
        ...
        eqn = [n_u_x[i] - R[i] for i in range(int(length))]
    ...
    def predict(self, x_test):
      ...
      n_test   = [self.sess.run(self.n_pred[i], tf_dict) for i in range(int(length))]
      ...
      R_test   = [self.sess.run(self.R_pred[i], tf_dict) for i in range(int(length))]
...
if __name__ == "__main__":
    ...
    #FIXME: if levels = even (2,4,..) the run stops, so there must be a bug
    # probably between lines 364:394 ... maybe not really dangerous
    levels = 1 # how many levels
    which_level = 27 # which level(s) [1:47]

    # NN Architecture
    layers = [1, 25, 50, 75, 50, 25, 2*levels+4]
    #layers = [1, 6, 12, 18, 24, 2*levels+4]
    #layers = [1, 100, 2*levels+4]
    ...
    #n  = [data[:,i+1:i+1+1].flatten()[:,None] for i in range(int(levels))] # starts from i=0 ... i=levels-1
    n   = [data[:,which_level:which_level+1].flatten()[:,None] for i in range(int(levels))]
    rho = data[:,49:50].flatten()[:,None]
    u   = data[:,50:51].flatten()[:,None]
    p   = data[:,51:52].flatten()[:,None]
    E   = data[:,52:53].flatten()[:,None]
    #R  = [data[:,i+53:i+54].flatten()[:,None] for i in range(int(levels))]
    R   = [data[:,which_level+53:which_level+54].flatten()[:,None] for i in range(int(levels))]
    ...
    while i<levels:
      ...
      i = i + 1
      j = j + 2
    rho_train = tts[levels-1+2*2]
    rho_test  = tts[levels-1+2*2+1]
    u_train   = tts[levels-1+3*2]
    u_test    = tts[levels-1+3*2+1]
    p_train   = tts[levels-1+4*2]
    p_test    = tts[levels-1+4*2+1]
    E_train   = tts[levels-1+5*2]
    E_test    = tts[levels-1+5*2+1]
    i = 0
    j = 0
    R_train = []
    R_test  = []
    while i<levels:
      R_train.append(tts[j+levels-1+6*2])
      R_test.append(tts[j+levels-1+6*2+1])
      i = i + 1
      j = j + 2

    sc_x = MinMaxScaler(); sc_x.fit(x_train); x_train = sc_x.transform(x_train)
    sc_n = [MinMaxScaler() for i in range(int(levels))]
    ...
    sc_R   = [MinMaxScaler() for i in range(int(levels))]
    ...
    error_n   = [np.linalg.norm(n_test[i] - n_pred[i], 2)/np.linalg.norm(n_test[i],2) for i in range(int(levels))]
    ...
    error_R   = [np.linalg.norm(R_test[i] - R_pred[i], 2)/np.linalg.norm(R_test[i],2) for i in range(int(levels))]
    ...
    n_train_sb   = [sc_n[i].inverse_transform(n_train[i]) for i in range(int(levels))]
    ...
    R_train_sb   = [sc_R[i].inverse_transform(R_train[i]) for i in range(int(levels))]
    x_test_sb    = sc_x.inverse_transform(x_test)
    n_test_sb    = [sc_n[i].inverse_transform(n_test[i]) for i in range(int(levels))]
    ...
    R_test_sb    = [sc_R[i].inverse_transform(R_test[i]) for i in range(int(levels))]
    n_pred_sb    = [sc_n[i].inverse_transform(n_pred[i]) for i in range(int(levels))]
    ...
    R_pred_sb    = [sc_R[i].inverse_transform(R_pred[i]) for i in range(int(levels))]

    # Plot Nci
    for i in range(levels):
        plt.plot(x_test_sb, n_pred_sb[i], 'o', color='black', label='NN, i=?', linewidth=4, markersize=5, fillstyle='none')
        plt.plot(x_test_sb, n_test_sb[i], 'o', color='red',   label='Exact, i=?', markersize=4)
        #plt.title(" ")
        plt.xlabel('X [mm]')
        plt.ylabel('$n_{ci}$ $[m^-3]$')
        #plt.legend()
        plt.tight_layout()
        plt.savefig(f"figures/Nci_nlevels", crop='false')
        plt.show()
        plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"figures/RHO_nlevels", crop='false')
    plt.show()
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"figures/P_nlevels", crop='false')
    plt.show()
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"figures/U_nlevels", crop='false')
    plt.show()
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"figures/E_nlevels", crop='false')
    plt.show()
    plt.close()

    # Plot Rci
    for i in range(levels):
        plt.plot(x_test_sb, R_pred_sb[i], 'o', color='black', label='NN, i=?', linewidth=4, markersize=5, fillstyle='none')
        plt.plot(x_test_sb, R_test_sb[i], 'o', color='red',   label='Exact, i=?',           markersize=4)
        plt.xlabel('X [mm]')
        plt.ylabel(r'$R_{ci} [J/m^3/s]$')
        plt.tight_layout()
        plt.savefig(f"figures/Rci_nlevels", crop='false')
        plt.show()
        plt.close()
