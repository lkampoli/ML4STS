...
const                       fdeep::tensor tensor(fdeep::tensor_shape(static_cast<std::size_t>(4)), std::vector<float>{1, 2, 3, 4});
const std::vector<float> vec = tensor.to_vector();
const std::vector<float> v = {1, 2, 3};
const fdeep::tensor t(fdeep::tensor_shape(3, 1, 1), v);
//const auto multi_inputs = fplus::generate<std::vector<fdeep::tensors>>([&]() -> fdeep::tensors {return model.generate_dummy_inputs();}, 10);
