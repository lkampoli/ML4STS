...
class PINN:
    ...
    def __init__(self, x, t, rho, u, p, E, layers):
        ...
        self.loss = tf.reduce_sum(tf.square(self.p_tf   - self.p_pred))  /(p_norm**2)   +                     tf.reduce_sum(tf.square(self.rho_tf - self.rho_pred))/(rho_norm**2) +                     tf.reduce_sum(tf.square(self.u_tf   - self.u_pred))  /(u_norm**2)   +                     tf.reduce_sum(tf.square(self.E_tf   - self.E_pred))  /(E_norm**2)   #+ \
