...
def mk_tree(algorithm, output_dir):
    
    models  = "models"
    scalers = "scalers"
    figures = "figures"

    model  = os.path.join(output_dir+"/"+algorithm, models)
    scaler = os.path.join(output_dir+"/"+algorithm, scalers)
    figure = os.path.join(output_dir+"/"+algorithm, figures)
...
def draw_plot(x_test_dim, y_test_dim, y_regr_dim, figure):
   
    plt.scatter(x_test_dim[:,0], y_test_dim[:,0], s=5,   c='k', marker='o', label='Matlab')
    plt.scatter(x_test_dim[:,0], y_regr_dim[:,0], s=2.5, c='r', marker='o', label='DecisionTree')
    #plt.ylabel(r'$\eta$ [Pa·s]')
    plt.xlabel('T [K] ')
    plt.legend()
    plt.tight_layout()
    plt.savefig(figure+"/regression.eps")
    #plt.show()
    plt.close()
