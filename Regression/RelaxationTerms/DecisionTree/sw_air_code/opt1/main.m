...
% Save dataset for MLA later use
%dataset = [X, Y];
%save solution_ODE_XY.dat dataset -ascii

% Load pre-computed Matlab solution (for speed)
%solution = load("sw_air_code_solution.mat");
%X = solution.X;
%Y = solution.Y;
...
%% Here starts the MLA part
 %figure, plot(x_s, Temp, 'r')
 %hold on;
 
%figure, plot(x_s, v, 'r')
%hold on;

%figure, plot(x_s, nn2_i(:,3), 'k')
%hold on;
%plot(x_s, nn2_i(:,6), 'b');
%plot(x_s, nn2_i(:,9), 'r');
%plot(x_s, nn2_i(:,12), 'g');
...
figure, loglog(x_s, nn2_i(:,1), 'k')
hold on;
loglog(x_s, no2_i(:,1), 'b');
loglog(x_s, nno_i(:,1), 'r');
loglog(x_s, nn(:,1), 'g');
loglog(x_s, no(:,1), 'm');

 % First variant: learn [X,Y] instead of ODE function call
 tic
 %my_xspan = linspace(0,x_w/Delta,10000000);
 my_xspan = logspace(-1,6,50);
 for i = 1:1:42
   input = my_xspan(i);
   XY = py.run_regression.regressor(input);
   XYd = double(XY);

   ni_N2_ML = XYd(1:l(1))             * n0;
   ni_O2_ML = XYd(l(1)+1:l(1)+l(2))   * n0;
   ni_NO_ML = XYd(l(1)+l(2)+1:sum(l)) * n0;
   n_N_ML   = XYd(sum(l)+1)           * n0;
   n_O_ML   = XYd(sum(l)+2)           * n0;
   V_ML     = XYd(sum(l)+3)           * v0;
   T_ML     = XYd(sum(l)+4)           * T0;
   ...
   scatter(input*Delta*100,ni_N2_ML(1),'k');
   scatter(input*Delta*100,ni_O2_ML(1),'b');
   scatter(input*Delta*100,ni_NO_ML(1),'r');
   scatter(input*Delta*100,n_N_ML,'g');
   scatter(input*Delta*100,n_O_ML,'m');
   set(gca,'xscale','log')
   set(gca,'yscale','log')
   
   %scatter(input*Delta*100,ni_N2_ML(3),'k');
   %scatter(input*Delta*100,ni_N2_ML(6),'r');
   %scatter(input*Delta*100,ni_N2_ML(9),'b');
   %scatter(input*Delta*100,ni_N2_ML(12),'g');
   %scatter(input*Delta*100,ni_N2_ML(15),'m');
 ...
 toc
   ...
   %ylabel('Velocity [m/s]')
   %legend({'Matlab','MLA'},'Location','southeast');
   legend({'Matlab N2, i=1','Matlab O2, i=1','Matlab NO, i=1', 'Matlab N', 'Matlab O', 'MLA N2, i=1','MLA O2, i=1','MLA NO, i=1', 'MLA N','MLA O'}, 'Location','southeast')
   %scatter(input*Delta*100,T_ML,'k');
   %scatter(input*Delta*100,ni_N2_ML(1),'k');
