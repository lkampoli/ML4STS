...
%	функция расчета правых частей для УВ
%	воздушная смесь
%   рассматриваются только 1-квантовые энергообмены

format long e
global c h k m l Be D n0 v0 T0 Delta sw_z;
global en2_i en2_0 eo2_i eo2_0 eno_i eno_0;

% число колебательных состояний молекул N2-O2-NO
l1 = l(1); l2 = l(2); l3 = l(3);
lall = l1+l2+l3;

% номер максимального колебательного уровня
...
% числовые плотности молекул N2-O2-NO
nn2_b = sum(nn2i_b);
no2_b = sum(no2i_b);
nno_b = sum(nnoi_b);

% общая числовая плотность молекул и атомов
na_b = nn_b+no_b;
nm_b = sum(y(1:lall));

% положение за фронтом, м
% xx = t*Delta;
% disp(xx)
...
% энергия образования атомов и молекул NO в безр. виде
efn_b = 0.5*D(1)/T0;
efo_b = 0.5*D(2)/T0;
...
% фактор симметрии молекул N2-O2-NO
sigma = [2 2 1];

% характеристическая вращательная температура молекулы, К
Theta_r = Be*h*c/k;

% колебательная статсумма молекул N2-O2-NO
Z_rot = temp./(sigma.*Theta_r);

% суммарная масса молекул и атомов, кг
M = sum(m);

% безразмерная масса частиц
mb = m/M;

%% составляем матрицу коэффициентов для системы A*X=B
A = zeros(sum(l)+4,sum(l)+4);

% кинетическое уравнение для молекул
for i=1:l1 % N2
    A(i,i) = v_b;
    A(i,lall + 3) = nn2i_b(i);
end
...
%% коэффициенты скорости процессов

% отношение коэффициентов прямых и обратных скоростей процессов:
% k_rec / k_dis, м^3
...
% коэффициенты скорости рекомбинации
...
% коэффициенты скорости реакций Зельдовича
if  strcmp(sw_z,'Savelev')
    % аппроксимация Савельева STELLAR database
...
elseif strcmp(sw_z,'Stellar')
    % прямая интерполяция STELLAR database
...
% коэффициенты скорости VT переходов i+1 -> i
...
% коэффициенты скорости VT переходов i -> i+1
...
% коэффициенты скорости VV обменов (i,j)->(i-1,j+1)
...
% коэффициенты скорости VV обменов (i-1,j+1)->(i,j)
...
for i1 = 1:l1
    ...
    if i1 == 1 % учитываются перходы 0<->1
...
%                      nn2i_b(i1)*(sum(nnoi_b(2:end) .* kvvs_u_n2_no(i1,:)'));
    ...
    elseif i1 == l1 % учитываются перходы Lmax <-> Lmax-1
...
for i2 = 1:l2
    ...
    if i2 == 1 % учитываются перходы 0<->1
...
%                      no2i_b(i2)*(sum(nnoi_b(2:end) .* kvvs_u_o2_no(i2,:)'));
    ...
    elseif i2 == l2 % учитываются перходы Lmax <-> Lmax-1
...
for i3 = 1:l3
    ...
    if i3 == 1 % учитываются перходы 0<->1
...
%                      nnoi_b(i3)*(sum(no2i_b(2:end) .* kvvs_u_no_o2(i3,:)'));
    ...
    elseif i3 == l3 % учитываются перходы Lmax <-> Lmax-1
...
%dataset = [nn2i_b; no2i_b; nnoi_b; nn_b; no_b; v_b*v0; T_b*T0; ...
%           RDn2; RZn2; RVTn2; RVVn2; ...
%           RDo2; RZo2; RVTo2; RVVo2; ...
%           RDno; RZno; RVTno; RVVno];
...
dataset = [nn2i_b; no2i_b; nnoi_b; nn_b; no_b; v_b*v0; T_b*T0; B];
save dataset.dat dataset -ascii -append
