...
input  = [nn2i_b; no2i_b; nnoi_b; nn_b; no_b; v_b*v0; T_b*T0];
B_ML   = py.run_regression.regressor(input);
B_ML_d = double(B_ML);

B = zeros(lall+4,1);
B(1:l1)         = B_ML_d(1:l1);
B(l1+1:l1+l2)   = B_ML_d(l1+1:l1+l2);
B(l1+l2+1:lall) = B_ML_d(l1+l2+1:lall);
B(lall+1)       = B_ML_d(lall+1);
B(lall+2)       = B_ML_d(lall+2);
