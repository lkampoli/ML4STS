...
x_folds = np.array_split(x_train, n_split)
y_folds = np.array_split(y_train, n_split)
scores = list()
for k in range(n_split):
    ...
    scores.append(regr.fit(X_train, Y_train).score(X_test, Y_test))
print(scores)

k_fold = KFold(n_splits=n_split)
score = cross_val_score(regr, x_train, y_train, cv=k_fold, n_jobs=-1)
print(score)
