...
!} TF_Code;
!
integer(c_int), parameter ::  TF_OK                  = 0
integer(c_int), parameter ::  TF_CANCELLED           = 1
integer(c_int), parameter ::  TF_UNKNOWN             = 2
integer(c_int), parameter ::  TF_INVALID_ARGUMENT    = 3
integer(c_int), parameter ::  TF_DEADLINE_EXCEEDED   = 4
integer(c_int), parameter ::  TF_NOT_FOUND           = 5
integer(c_int), parameter ::  TF_ALREADY_EXISTS      = 6
integer(c_int), parameter ::  TF_PERMISSION_DENIED   = 7
integer(c_int), parameter ::  TF_UNAUTHENTICATED     = 1
integer(c_int), parameter ::  TF_RESOURCE_EXHAUSTED  = 8
integer(c_int), parameter ::  TF_FAILED_PRECONDITION = 9
integer(c_int), parameter ::  TF_ABORTED             = 10
integer(c_int), parameter ::  TF_OUT_OF_RANGE        = 11
integer(c_int), parameter ::  TF_UNIMPLEMENTED       = 12
integer(c_int), parameter ::  TF_INTERNAL            = 13
integer(c_int), parameter ::  TF_UNAVAILABLE         = 14
integer(c_int), parameter ::  TF_DATA_LOSS           = 15
...
!                TF_DATA_LOSS           = 15
!end enum
!integer, parameter, public :: TF_Code = kind(TF_OK)
!public :: TF_OK, TF_CANCELLED, TF_UNKNOWN, TF_INVALID_ARGUMENT, &
...
!          TF_INTERNAL, TF_UNAVAILABLE, TF_DATA_LOSS
!// --------------------------------------------------------------------------
!
...
interface
subroutine TF_SetStatus(s, code, msg) bind(C, name="TF_SetStatus")
  import TF_Status, TF_Code, c_char
  implicit none
  type(TF_Status), value :: s
  type(TF_Code), value :: code
...
!                                                   const char* context);
interface
  subroutine TF_SetStatusFromIOError(s, error_code, context) bind(C, name="TF_SetStatusFromIOError")
    import TF_Status, c_int, c_char
    implicit none
    type(TF_Status), value :: s
    integer(c_int) :: error_code
    character(c_char) :: context
end subroutine
end interface
!
...
interface
function TF_GetCode(s) bind(C, name="TF_GetCode") result(TF_Code_)
  import TF_Status, TF_Code
  implicit none
  type(TF_Status), value :: s
  type(TF_Code) :: TF_Code_
end function
end interface
!
...
!} TF_DataType;
!
integer(c_int), parameter :: TF_FLOAT      = 1
integer(c_int), parameter :: TF_DOUBLE     = 2
integer(c_int), parameter :: TF_INT32      = 3  ! Int32 tensors are always in 'host' memory.
integer(c_int), parameter :: TF_UINT8      = 4
integer(c_int), parameter :: TF_INT16      = 5
integer(c_int), parameter :: TF_INT8       = 6
integer(c_int), parameter :: TF_STRING     = 7
integer(c_int), parameter :: TF_COMPLEX64  = 8  ! Single-precision complex
integer(c_int), parameter :: TF_COMPLEX    = 8  ! Old identifier kept for API backwards compatibility
integer(c_int), parameter :: TF_INT64      = 9
integer(c_int), parameter :: TF_BOOL       = 1
integer(c_int), parameter :: TF_QINT8      = 11 ! Quantized int8
integer(c_int), parameter :: TF_QUINT8     = 12 ! Quantized uint8
integer(c_int), parameter :: TF_QINT32     = 13 ! Quantized int32
integer(c_int), parameter :: TF_BFLOAT16   = 14 ! Float32 truncated to 16 bits.  Only for cast ops.
integer(c_int), parameter :: TF_QINT16     = 15 ! Quantized int16
integer(c_int), parameter :: TF_QUINT16    = 16 ! Quantized uint16
integer(c_int), parameter :: TF_UINT16     = 17
integer(c_int), parameter :: TF_COMPLEX128 = 18 ! Double-precision complex
integer(c_int), parameter :: TF_HALF       = 19
integer(c_int), parameter :: TF_RESOURCE   = 20
integer(c_int), parameter :: TF_VARIANT    = 21
integer(c_int), parameter :: TF_UINT32     = 22
integer(c_int), parameter :: TF_UINT64     = 23
!
type, bind(C) :: TF_DataType
end type
!
!enum, bind(C)
...
interface
function TF_DataTypeSize(dt) bind(C, name="TF_DataTypeSize") result(TF_DataTypeSize_)
  import TF_DataType, c_size_t
  implicit none
  type(TF_DataType), value :: dt
  integer(c_size_t) :: TF_DataTypeSize_
end function
end interface
!
!#ifdef __cplusplus
...
!TF_CAPI_EXPORT extern TF_SessionOptions* TF_NewSessionOptions(void);a
interface
...
subroutine TF_SetTarget(options, target) bind(C, name="TF_SetTarget")
  ...
  type(TF_SessionOptions), value :: options
end subroutine
end interface
!
...
subroutine TF_SetConfig(options, proto, proto_len, status) bind(C, name="TF_SetConfig")
    import TF_SessionOptions, c_funptr, c_size_t, TF_Status
    implicit none
    type(TF_SessionOptions), value :: options
    type(c_funptr) :: proto
...
!                                                   TF_Status* status);
interface
function TF_GraphGetTensorNumDims(graph, output, status) bind(C, name="TF_GraphGetTensorNumDims") result(TF_GraphGetTensorNumDims_)
  import c_int, TF_Graph, TF_Output, TF_Status
  implicit none
  type(TF_Graph)  :: graph
  type(TF_Output) :: output
  type(TF_Status) :: status
  integer(c_int) :: TF_GraphGetTensorNumDims_
end function
end interface
...
interface
function TF_GraphOperationByName(graph, oper_name) bind(C, name="TF_GraphOperationByName") result(TF_Operation_)
  import TF_Graph, c_char, TF_Operation
  implicit none
  type(TF_Graph), value :: graph
  character(c_char) :: oper_name
  type(TF_Operation) :: TF_Operation_
end function
end interface
!
...
interface
subroutine TF_CloseSession(TF_Session_, status)
  import TF_Session, TF_Status
  implicit none
  type(TF_Session), value :: TF_Session_
  type(TF_Status), value :: status
end subroutine
end interface
!
...
!    TF_Status*);
interface
subroutine TF_SessionRun(session, run_options, &
                         inputs, input_values, ninputs, &
                         outputs, output_values, noutputs, &
                         target_opers, ntargets, &
                         run_metadata, status)
import TF_Session, TF_Buffer, TF_Output, TF_Tensor, c_int, &
       TF_Output, TF_Operation, TF_Status
implicit none
  type(TF_Session), value :: session
  type(TF_Buffer), value :: run_options, run_metadata
  type(TF_Output), value :: inputs, outputs
  type(TF_Tensor), value :: input_values, output_values
  integer(c_int) :: ninputs, noutputs, ntargets
  type(TF_Operation) :: target_opers
  type(TF_Status) :: status
end subroutine
end interface
!
...
interface
function TF_NewTensor(TF_DataType_, dims, num_dims, data, len, deallocator, deallocator_arg) bind(C, name="TF_NewTensor") &
    result(TF_Tensor_)
  import TF_Tensor, TF_DataType, c_int64_t, c_int, c_funptr, c_size_t
  implicit none
  type(TF_Tensor), value :: TF_DataType_
  integer(c_int64_t) :: dims
  integer(c_int) :: num_dims
  type(c_funptr), value :: data
  integer(c_size_t) :: len
  type(c_funptr), value :: deallocator
  type(c_funptr), value :: deallocator_arg
  type(TF_Tensor) :: TF_Tensor_
end function
end interface
!FIXME:
...
interface
subroutine TF_TensorData(TF_Tensor_) bind(C, name="TF_TensorData")
  import TF_Tensor
  implicit none
  type(TF_Tensor), value :: TF_Tensor_
end subroutine
end interface
!
! Destroy a tensor.
...
interface
subroutine TF_DeleteTensor(TF_Tensor_) bind(C, name="TF_DeleteTensor")
  import TF_Tensor
  implicit none
  type(TF_Tensor), value :: TF_Tensor_
end subroutine
